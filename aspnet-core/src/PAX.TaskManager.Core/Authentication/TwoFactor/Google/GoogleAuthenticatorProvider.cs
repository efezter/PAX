...
namespace PAX.TaskManager.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : TaskManagerServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("PAX.TaskManager", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
