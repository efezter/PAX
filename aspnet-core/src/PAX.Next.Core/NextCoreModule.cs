...
using PAX.Next.Authorization.Ldap;
using PAX.Next.Authorization.Roles;
using PAX.Next.Authorization.Users;
using PAX.Next.Chat;
using PAX.Next.Configuration;
using PAX.Next.DashboardCustomization.Definitions;
using PAX.Next.Debugging;
using PAX.Next.DynamicEntityProperties;
using PAX.Next.Features;
using PAX.Next.Friendships;
using PAX.Next.Friendships.Cache;
using PAX.Next.Localization;
using PAX.Next.MultiTenancy;
using PAX.Next.Net.Emailing;
using PAX.Next.Notifications;
using PAX.Next.WebHooks;
using Newtonsoft.Json;

namespace PAX.Next
{
    [DependsOn(
        typeof(NextCoreSharedModule),
        typeof(AbpZeroCoreModule),
        typeof(AbpZeroLdapModule),
        typeof(AbpAutoMapperModule),
        typeof(AbpAspNetZeroCoreModule),
        typeof(AbpMailKitModule))]
    public class NextCoreModule : AbpModule
    {
        public override void PreInitialize()
        {
            ...
            NextLocalizationConfigurer.Configure(Configuration.Localization);
            ...
            Configuration.MultiTenancy.IsEnabled = NextConsts.MultiTenancyEnabled;
            ...
            Configuration.ReplaceService<IMailKitSmtpBuilder, NextMailKitSmtpBuilder>(DependencyLifeStyle.Transient);
            ...
            Configuration.ReplaceService(typeof(IEmailSenderConfiguration), () =>
            {
                Configuration.IocManager.IocContainer.Register(
                    Component.For<IEmailSenderConfiguration, ISmtpEmailSenderConfiguration>()
                             .ImplementedBy<NextSmtpEmailSenderConfiguration>()
                             .LifestyleTransient()
                );
            });
        ...
        {
            IocManager.RegisterAssemblyByConvention(typeof(NextCoreModule).GetAssembly());
        }
