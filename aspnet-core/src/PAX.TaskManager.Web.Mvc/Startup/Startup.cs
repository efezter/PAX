...
using PAX.TaskManager.Configure;
using PAX.TaskManager.EntityFrameworkCore;
using PAX.TaskManager.Identity;
using PAX.TaskManager.Schemas;
using PAX.TaskManager.Web.Chat.SignalR;
using PAX.TaskManager.Web.Common;
using PAX.TaskManager.Web.Resources;
using Swashbuckle.AspNetCore.Swagger;
using PAX.TaskManager.Web.IdentityServer;
using PAX.TaskManager.Web.Swagger;
using Stripe;
...
using PAX.TaskManager.Web.HealthCheck;
using Owl.reCAPTCHA;
using HealthChecksUISettings = HealthChecks.UI.Configuration.Settings;
using Microsoft.AspNetCore.Server.Kestrel.Https;
using Microsoft.Extensions.DependencyInjection.Extensions;
using SecurityStampValidatorCallback = PAX.TaskManager.Identity.SecurityStampValidatorCallback;
...
namespace PAX.TaskManager.Web.Startup
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            return services.AddAbp<TaskManagerWebMvcModule>(options =>
            {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory)
        {
            ...
            if (WebConsts.SwaggerUiEnabled)
            {
                ...
                {
                    options.SwaggerEndpoint(_appConfiguration["App:SwaggerEndPoint"], "TaskManager API V1");
                    options.IndexStream = () => Assembly.GetExecutingAssembly()
                        .GetManifestResourceStream("PAX.TaskManager.Web.wwwroot.swagger.ui.index.html");
        ...
        private void ConfigureSwagger(IServiceCollection services)
        {
            services.AddSwaggerGen(options =>
            {
                ...
                var applicationXml = $"PAX.TaskManager.Application.xml";
                var applicationXmlPath = Path.Combine(AppContext.BaseDirectory, applicationXml);
                options.IncludeXmlComments(applicationXmlPath);

                var webCoreXmlFile = $"PAX.TaskManager.Web.Core.xml";
