...
using PAX.Next.Migrations.Seed.Host;
...
namespace PAX.Next.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : NextAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<NextDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{NextConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{NextConsts.ConnectionStringName}", constring);
        }
