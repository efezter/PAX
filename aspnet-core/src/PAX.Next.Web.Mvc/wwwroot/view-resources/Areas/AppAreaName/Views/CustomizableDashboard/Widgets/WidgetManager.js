    ...
    app.WidgetManager = (function () {
        ...
        return function (options) {
            ...
            function initialize() {
                ...
                if (closestPage.hasClass("active")) {//if the widget is in active tab, load it immediately
                    initAndShowWidget();
                } else {//otherwise wait for the tab to be active for the widget to load
                    var closesPagesId = closestPage.attr('id');

                    $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
                        var target = $(e.target).attr("href");
                        if (target === "#" + closesPagesId) {//if widget's page is activated, load widget
                            initAndShowWidget();
                        }
                    });
                }
