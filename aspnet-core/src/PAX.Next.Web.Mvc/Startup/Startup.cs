...
using PAX.Next.Configure;
using PAX.Next.EntityFrameworkCore;
using PAX.Next.Identity;
using PAX.Next.Schemas;
using PAX.Next.Web.Chat.SignalR;
using PAX.Next.Web.Common;
using PAX.Next.Web.Resources;
using Swashbuckle.AspNetCore.Swagger;
using PAX.Next.Web.IdentityServer;
using PAX.Next.Web.Swagger;
using Stripe;
...
using PAX.Next.Web.HealthCheck;
using Owl.reCAPTCHA;
using HealthChecksUISettings = HealthChecks.UI.Configuration.Settings;
using Microsoft.AspNetCore.Server.Kestrel.Https;
using Microsoft.Extensions.DependencyInjection.Extensions;
using SecurityStampValidatorCallback = PAX.Next.Identity.SecurityStampValidatorCallback;
...
namespace PAX.Next.Web.Startup
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            return services.AddAbp<NextWebMvcModule>(options =>
            {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory)
        {
            ...
            if (WebConsts.SwaggerUiEnabled)
            {
                ...
                {
                    options.SwaggerEndpoint(_appConfiguration["App:SwaggerEndPoint"], "Next API V1");
                    options.IndexStream = () => Assembly.GetExecutingAssembly()
                        .GetManifestResourceStream("PAX.Next.Web.wwwroot.swagger.ui.index.html");
        ...
        private void ConfigureSwagger(IServiceCollection services)
        {
            ...
            {
                options.SwaggerDoc("v1", new OpenApiInfo() {Title = "Next API", Version = "v1"});
                ...
                var applicationXml = $"PAX.Next.Application.xml";
                var applicationXmlPath = Path.Combine(AppContext.BaseDirectory, applicationXml);
                options.IncludeXmlComments(applicationXmlPath);

                var webCoreXmlFile = $"PAX.Next.Web.Core.xml";
