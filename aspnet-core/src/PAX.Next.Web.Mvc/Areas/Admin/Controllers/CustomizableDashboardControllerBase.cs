...
using PAX.Next.Web.Areas.Admin.Models.CustomizableDashboard;
...
{
    public abstract class CustomizableDashboardControllerBase : NextControllerBase
    {
        ...
        protected async Task<ActionResult> GetView(string dashboardName)
        {
            var dashboardDefinition = DashboardCustomizationAppService.GetDashboardDefinition(
                ...
                {
                    DashboardName = dashboardName,
                    Application = NextDashboardCustomizationConsts.Applications.Mvc
                }
            );
