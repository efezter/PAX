...
import {TaskStatusesServiceProxy, UpdateIconInput} from '@shared/service-proxies/service-proxies';
...
export class ChangeIconModalComponent extends AppComponentBase {
    ...
    updateProfilePicture(fileToken: string): void {
        ...
        input.width = 0;
        input.height = 0;
        input.taskStatusId = this.item.taskStatus.id;

        this.saving = true;
        this._taskStatusesServiceProxy.updateIcon(input)
            .pipe(finalize(() => {
                this.saving = false;
            }))
            .subscribe(() => {
